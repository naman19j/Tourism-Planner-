...
public class hotelpayment1 extends HttpServlet {
...
String s6=request.getParameter("arrive");
String s7=request.getParameter("depart");
String s8=request.getParameter("people");
String s10=request.getParameter("bedding");


        
stmt.executeUpdate("insert into bookdetails values('"+s1+"','"+s2+"','"+s3+"','"+s4+"','"+s5+"','"+s6+"','"+s7+"','"+s8+"','"+s10+"')");
int amount=0;
if("single-bed".equals(s10)){
    amount=amount+999;
}
else if("double-bed".equals(s10)){
    amount+=1299;
}
else if("deluxe-suite".equals(s10)){
    amount+=2999;
}
else if("terrace-suite".equals(s10)){
    amount+=3999;
}
else if("penthouse-suite".equals(s10)){
    amount+=4999;
}
else if("bunk-bed".equals(s10)){
    amount+=599;
}
LocalDate dateBefore = LocalDate.parse(s6);
LocalDate dateAfter = LocalDate.parse(s7);
int noOfDaysBetween = (int) ChronoUnit.DAYS.between(dateBefore, dateAfter);
amount=amount*noOfDaysBetween;
...
"  <div class=\"col-75\">\n" +
"    <div class=\"container\">\n" +
"      <form action=\"bookcnfm\" method=\"post\">\n" +
"      \n" +
"        <div class=\"row\">\n" +
"          <div class=\"col-50\">\n" +
...
{
Logger.getLogger(hotelpayment1.class.getName()).log(Level.SEVERE, null, ex);
}
}
}
